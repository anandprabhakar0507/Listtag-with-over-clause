REM   Script: Analytics - LISTAGG with the OVER clause
REM   SQL from the KISS (Keep It Simply SQL) Analytic video series by Developer Advocate Connor McDonald. This script is for looking at to how use an analytic expression on the listagg function
...
create table planets ( name varchar2(20) primary key, class varchar2(20) );

insert into planets values ('Mercury','Inner');

insert into planets values ('Venus','Inner');

insert into planets values ('Earth','Earth');

insert into planets values ('Eris','Dwarf');

insert into planets values ('Haumea','Dwarf');

insert into planets values ('Jupiter','Superior');

insert into planets values ('Mars','Superior');

insert into planets values ('Neptune','Superior');

insert into planets values ('Pluto','Superior');

insert into planets values ('Saturn','Superior');

insert into planets values ('Uranus','Superior');
...
insert into moons values ('Haumea','Hiiaka',195);
...
create or replace view named_moons as  
select p.name planet, p.class, m.name moon, m.radius 
...
select class,  
       planet, 
       min(radius) min_r, 
       listagg(radius,',') within group ( order by planet ) members 
from named_moons 
group by class, planet;

select class,  
       planet, 
       min(radius), 
       listagg(min(radius),',') within group ( order by planet ) 
         over ( partition by class ) class_radii 
from named_moons 
group by class, planet 
order by 1,2;
